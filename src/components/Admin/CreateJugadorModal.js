...
const CreateJugadorModal = ({ onClose, onJugadorCreated }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    setLoading(true);
    try {
      const res = await fetch('http://localhost:8000/jugadors', {
        method: 'POST',
      ...
      if (!res.ok) {
        ...
        throw new Error(err.detail || 'Error al crear el jugador');
      }

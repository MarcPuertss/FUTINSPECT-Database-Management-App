// src/components/Admin/AdminPartitsPage.js
import React, { useState, useEffect } from 'react';
import { getPartits } from '../../api/apiService';
import AdminPartitCard from './AdminPartitCard';
import Loading from '../Common/Loading';
import CreatePartitModal from './CreatePartitModal';

const AdminPartitsPage = () => {
  ...
  // Función para manejar la creación de un nuevo partido
  const handlePartitCreated = () => {
    loadPartits(); // Recargar la lista de partidos
    ...
    <div style={{ padding: '32px', maxWidth: '1400px', margin: '0 auto' }}>
      ...
      <div style={{
        ...
        <div style={{
          ...
          {(searchTerm || statusFilter !== 'all' || year || month) && (
            <button
              onClick={() => {
                setSearchTerm('');
                setStatusFilter('all');
      ...
      {filteredPartits.length > 0 ? (
        <div style={{
          ...
          {filteredPartits.map((partit, index) => (
            <AdminPartitCard key={`${partit.id_partit || index}`} partit={partit} />
          ))}
        </div>
      ) : (
        <div style={{
      ...
      {showCreateModal && (
        <CreatePartitModal
          ...
          onPartitCreated={handlePartitCreated}
        />
      )}
    </div>
  );
...
export default AdminPartitsPage;